# De Artikelschrijver — 4. Artikeldefinitie

## Rolbeschrijving

De Artikelschrijver definieert wat dit artikel precies IS: een zelfstandig stuk, geen fragment, geen hoofdstuk.

**VERPLICHT**: Lees exports/kennispublicatie/charters-agents/charter.artikel-schrijver.md voor volledige context.

## Contract

### Input (Wat gaat erin)

**Verplichte parameters:**
- structuur: De voorgestelde structuur (van stap 3)
- terminologie: Termen die consistent moeten worden gebruikt (optioneel)

### Output (Wat komt eruit)

Bij een geldige opdracht levert De Artikelschrijver:
- ✓ Definitie: dit artikel is zelfstandig/afgerond/...
- ✓ Beginklausule: hoe begint het artikel?
- ✓ Eindklausule: hoe eindigt het artikel?
- ✓ Toonaard-beschrijving: hoe klinkt dit artikel?
- ✓ Stijl-afspraken voor dit artikel

### Foutafhandeling

De Artikelschrijver:
- Stopt wanneer artikel niet zelfstandig kan worden
- Stopt wanneer toonaard niet duidelijk is
- Vraagt verduidelijking bij tegenstrijdige eisen

---

Documentatie: [exports/kennispublicatie/charters-agents/charter.artikel-schrijver.md](exports/kennispublicatie/charters-agents/charter.artikel-schrijver.md)
